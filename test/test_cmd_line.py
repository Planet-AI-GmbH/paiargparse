...
class TestCmdLine(unittest.TestCase):
    ...
    def test_hierarchical_dc(self):
        ...
        out = check_output(
            [sys.executable, "run_hierarchical.py", "--root.child", "examples.structures.hierarchical:Child2"]
        )
        p = Parent.from_json(out)
    ...
    def test_flat_hierarchical_dc(self):
        ...
        out = check_output(
            [sys.executable, "run_flat.py", "--req_int", "5", "--child", "examples.structures.flat:Child2"]
        )
        p = Parent.from_json(out)
        self.assertIsInstance(p.child, Child2)
    ...
    def test_mixed(self):
        ...
        out = check_output(
            [
                ...
                "positional_arg",
                "--root.child",
                "examples.structures.hierarchical:Child2",
            ]
        )
        p = Parent.from_dict(json.loads(out)["root"])
        self.assertIsInstance(p.child, Child2)
